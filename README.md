...
**Status:** Phase 1 (MVP) and Phase 2 (standing guards + continuous checks)
complete. Not yet published to PyPI.
...
The table below is generated by `pytest tests/failure_injection` — six
...
and a longer-running recurring-date scenario built specifically to
demonstrate guard promotion), each run once with **no** recovery mechanism
(baseline) and once wrapped with ResilientForge, with different literal
values per trial:

| Scenario | Trials | Baseline recovery | Recovery (ResilientForge) | Avg attempts to recovery | Oracle hit rate (after 1st occurrence) | Guard promoted | Prevention rate |
|---|---|---|---|---|---|---|---|
| malformed_json_args | 5 | 0% | 100% | 0.6 | 100% | yes | 100% |
| missing_required_field | 5 | 0% | 100% | 0.6 | 100% | yes | 100% |
| natural_language_date | 5 | 0% | 100% | 0.6 | 100% | yes | 100% |
| transient_timeout | 5 | 0% | 100% | 1.0 | 100% | no | 0% |
| wrong_type_argument | 5 | 0% | 100% | 0.6 | 100% | yes | 100% |
| recurring_date_guard | 8 | 0% | 100% | 0.4 | 100% | yes | 100% |
...
calls**. The last two columns are Phase 2's proof: once a fix has been
...
recovered from. Regenerate this table yourself with `pytest -s
tests/failure_injection` (also written to
