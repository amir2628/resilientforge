#!/usr/bin/env python3
"""Round 2 validation driver — see ../../docs/real_world_validation_round2.md.
Independent of round 1's driver: own oracle path, own metrics log, own
session log directory, all inside validation/round2/.

Call this once per session (at least 5 required — see round 2's task
description for why sessions should be spread across real time gaps where
practical, not run strictly back-to-back). Oracle and metrics log persist
at fixed paths across invocations.

No oracle pre-seeding, no prompts hand-crafted to trigger a specific
failure — see prompts.py's module docstring.
"""
...
async def main() -> None:
    ...
    print(f"=== Round 2 validation session {session_num} ===")
